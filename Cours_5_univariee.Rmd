---
title: "M1 Carto - Analyse univariée"
...
requiredPackages = c('knitr','datasets','kableExtra','ggplot2','plotly','widgetframe','sn','reshape','dplyr','dotenv')
...
# Cours n°5- l'analyse univariée<br /> <br />
### Florian Bayer
...
beaver3 <- data.frame("temp" = rsn(n=95, xi=0, omega=0.10, alpha=0)+37,
                      "time" = sample(840:2500,95,replace=T),
                      "activ" = 0,
                      "day" = 307,
                      "castor" = "castor 3")

beaver4 <- data.frame("temp" = rsn(n=111, xi=0, omega=0.25, alpha=5)+36.5,
                      "time" = sample(840:2500,111,replace=T),
                      "activ" = 0,
                      "day" = 307,
                      "castor" = "castor 4")

beaver5 <- data.frame("temp" = rsn(n=105, xi=0, omega=0.25, alpha=-5)+38,
                      "time" = sample(840:2500,105,replace=T),
                      "activ" = 0,
                      "day" = 307,
                      "castor" = "castor 5")
df.castor <- bind_rows(beaver1, beaver2,beaver3,beaver4,beaver5) %>% 
...
p <- ggplot(df.castor%>% dplyr::filter(df.castor$castor %in% c("castor 1", "castor 2")), aes(time, temp,color=castor)) +
...
.center-img[
```{r echo=FALSE, out.width="70%"}
knitr::include_graphics("./images/5-Stat/plotly-example-plots.png")
```
] 
...
df.castor.mean <- df.castor %>% group_by(castor) %>%
  summarize(Moyenne = round(mean(temp),1), Min = round(min(temp),1), Max = round(max(temp),1))
...
p<- ggplot(df.castor.mean, aes(x = castor, y = Moyenne, ymin = Min, ymax = Max, fill = castor)) + 
  geom_bar(stat = "identity",width = 0.5) + 
  geom_errorbar(width = 0.3)+ ggtitle("t° moyenne des castors") +
  xlab("") + ylab("température °C")+ scale_y_continuous(breaks=seq(0, 40, by=5))+ 
...
p <- ggplot(df.castor%>% dplyr::filter(df.castor$castor %in% c("castor 1", "castor 2")),
...
p <- ggplot(df.castor%>% dplyr::filter(df.castor$castor %in% c("castor 1", "castor 2")),
            aes(x=temp,fill = castor, color = castor))+
  geom_histogram(alpha=0.5, binwidth  = 0.1,boundary = 0, closed = "left") + ggtitle("Classes de 0.1° d'amplitude") +
  xlab("Classes de température") + ylab("Fréquence") + theme(legend.position = "none") + ylim(0,70)+
  facet_grid(castor ~ .) 
...
knitr::include_graphics("./images/5-Stat/histo-barre.png")
```
] 
...
ggplot(df.castor%>% dplyr::filter(df.castor$castor %in% c("castor 1", "castor 2")),
       aes(x=temp,fill = castor, color = castor))+
  geom_histogram(alpha=0.5, binwidth  = 0.25,boundary = 0, closed = "left") + 
  xlab("Classes de température") + ylab("Fréquence") + theme(legend.position = "none") + ylim(0,70)+
  facet_grid(castor ~ .) + 
...
ggplot(df.castor%>% dplyr::filter(df.castor$castor %ni% c("castor 1", "castor 2")), aes(x=temp,fill = castor, color = castor))+
  geom_histogram(alpha=0.5,boundary = 0, closed = "left") + 
...
.center-img[
```{r echo=FALSE, out.width="70%"}
knitr::include_graphics("./images/5-Stat/billou.png")
```
] 
...
df.castor$castor_means <- ifelse(df.castor$castor=="castor 1", castor_means[1], castor_means[2])

castor_medians <- tapply(df.castor$temp, df.castor$castor, median)
df.castor$castor_medians <- ifelse(df.castor$castor=="castor 1", castor_medians[1], castor_medians[2])
...
ggplot(df.castor%>% dplyr::filter(df.castor$castor %in% c("castor 1", "castor 2")),
       aes(x=temp,fill = castor, color = castor))+
  geom_histogram(alpha=0.5, binwidth  = 0.1,boundary = 0, closed = "left") + ggtitle("moyenne et médiane") +
  xlab("Classes de température") + ylab("Fréquence") + theme(legend.position = "none") + ylim(0,30)+
  geom_vline(data=df.castor%>% dplyr::filter(df.castor$castor %in% c("castor 1", "castor 2")),
...
.center-img[
```{r echo=FALSE, out.width="70%"}
knitr::include_graphics("./images/5-Stat/centrale.png")
```
]
...
df.EAM <- df.EAM %>% rename("Etudiants i"  = Etudiants.i, "|Xi-Xmoy|" = X.Xi.Xmoy.)
...
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/loi_normale.png")
```
]]
...
.center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/normal.gif")
```
]
...
.center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/var_1.png")
```
]
...
.center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/var_2.png")
```
]
...
.center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/var_3.png")
```
]
...
.center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/var_4.png")
```
]
...
.center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/var_5.png")
```
]
...
.center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/var_6.png")
```
]
...
.center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/var_7.png")
```
]
...
.center-img[
```{r echo=FALSE, out.width="90%"}
knitr::include_graphics("./images/5-Stat/quantiles.png")
```
] 
...
.pull-right[.center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/boxplot.png")
```
]]
...
knitr::include_graphics("./images/5-Stat/skew.png")
```
] ]
...
.pull-right[ .center-img[
```{r echo=FALSE, out.width="100%"}
knitr::include_graphics("./images/5-Stat/applat.png")
```
] ]
...
Il existe également des tests statistiques pour déterminer si une série suit une loi particulière. Par exemple pour la loi normale, on peut utiliser un test de Jarque-Bera. Il se base sur le coefficient d'asymétrie et le kurtosis.
...
La statistique JB se teste via une loi du $\chi^2$ à deux degrés de liberté. On pose généralement H0 les données suivent une loi normale et H1 les données ne suivent pas une loi normale. Si les données suivent une loi normale, le test s'approche alors de 0 et on ne rejette pas Ho au seuil $\alpha$. 
...
Ce sont vos capacités à les interpréter dans le contexte de votre étude qui prime !
